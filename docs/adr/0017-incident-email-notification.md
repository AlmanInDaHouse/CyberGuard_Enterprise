...
The notification hangs off the incident write the detection pipeline already performs: `upsertIncident` (`services/ingest/src/detect/incidents.ts:66-101`), called from `runDetectionCycle` (`services/ingest/src/detect/index.ts:24`) right after a genuinely-new alert is persisted (`index.ts:38-55`, guarded by `alertId !== null`). That cycle has **no production caller** today — it runs only under the detection harness (`detect_ac_*`, `incident_ac_*`); its production driver is the in-process TypeScript scheduler added by ADR-0012 Amendment 2026-06-07 (`0012-normalize-before-correlate-pipeline.md:271`); the Go `services/pipeline/` extraction remains deferred to the firehose ADR (`0012-normalize-before-correlate-pipeline.md:236`), decoupled from the prod-caller role. This ADR is honest about that: it fixes *where* and *how* notification attaches, at the altitude the rest of the detection MVP is validated (test-driven), not as an email running in production today.
...
- **Test-validated altitude only.** Because `runDetectionCycle` has no production caller yet (ADR-0012 §1, `0012-normalize-before-correlate-pipeline.md:28`), notification does not fire in a deployed system today. It is wired at the correct point and exercised end-to-end through the detection harness; when the in-process TypeScript scheduler drives the cycle in production (ADR-0012 Amendment 2026-06-07, `:271`), notification activates **without re-touching the seam**. This inherited gap is pre-existing and shared by the whole detection MVP — this ADR neither widens nor resolves it.
...
- **The production detection driver** — the in-process TypeScript scheduler in `services/ingest` that gives `runDetectionCycle` its production caller, specified by ADR-0012 Amendment 2026-06-07 (`0012-normalize-before-correlate-pipeline.md:271`). Inherited gap; notification rides whatever drives the cycle. (The Go `services/pipeline/` extraction / event firehose remains deferred to the firehose ADR — `0012-normalize-before-correlate-pipeline.md:236` — but is decoupled from the prod-caller role.)
