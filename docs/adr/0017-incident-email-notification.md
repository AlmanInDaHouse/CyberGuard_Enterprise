...
The notification hangs off the incident write the detection pipeline already performs: `upsertIncident` (`services/ingest/src/detect/incidents.ts:66-101`), called from `runDetectionCycle` (`services/ingest/src/detect/index.ts:24`) right after a genuinely-new alert is persisted (`index.ts:38-55`, guarded by `alertId !== null`). That cycle now has a **production caller** — it no longer runs only under the detection harness (`detect_ac_*`, `incident_ac_*`); its production driver is the in-process TypeScript scheduler added by ADR-0012 Amendment 2026-06-07 (`0012-normalize-before-correlate-pipeline.md:271`); the Go `services/pipeline/` extraction remains deferred to the firehose ADR (`0012-normalize-before-correlate-pipeline.md:236`), decoupled from the prod-caller role. This ADR fixes *where* and *how* notification attaches; with the prod-driver merged, the rest of the detection MVP runs in a standing stack and notification fires in a deployed system.
...
- **Test-validated altitude — resolved by the prod-driver.** `runDetectionCycle` now has a production caller — the in-process TypeScript scheduler that drives the cycle in production (ADR-0012 Amendment 2026-06-07, `0012-normalize-before-correlate-pipeline.md:271`) — so notification fires in a deployed system. It is wired at the correct point and exercised end-to-end through the detection harness, and notification activates **without re-touching the seam**. This inherited gap was pre-existing and shared by the whole detection MVP; it is resolved by the prod-driver merge, not by this ADR (which neither widened nor resolved it).
...
- **The production detection driver** — the in-process TypeScript scheduler in `services/ingest` that gives `runDetectionCycle` its production caller, specified by ADR-0012 Amendment 2026-06-07 (`0012-normalize-before-correlate-pipeline.md:271`). Inherited gap, now closed by the prod-driver; notification rides whatever drives the cycle. (The Go `services/pipeline/` extraction / event firehose remains deferred to the firehose ADR — `0012-normalize-before-correlate-pipeline.md:236` — but is decoupled from the prod-caller role.)
...
- [ADR-0012](0012-normalize-before-correlate-pipeline.md) — the transitory TypeScript detect seam (`:28`, `:213`), the in-process TypeScript scheduler that drives `runDetectionCycle` in production (Amendment 2026-06-07, `:271`) — the driver the notify rides — and the deferred Go `services/pipeline/` firehose (`:236`), decoupled from the prod-caller role.
