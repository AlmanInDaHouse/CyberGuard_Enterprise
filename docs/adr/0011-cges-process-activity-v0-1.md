...
- SPEC-005 (forthcoming) — First consumer of this ADR. Specifies agent-side ETW capture, the kernel-device-path → Win32 conversion mechanism, the v0.1 emission scope, the AC tests pinning §6's byte-level format, and the CommandLine PII accepted-risk decision.
