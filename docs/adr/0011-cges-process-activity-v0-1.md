...
**Cross-event stability.** Launch and Terminate of one process instance produce the same `process.uid` because both use the same `(PID, creation_time)` tuple — the creation_time is invariant across an instance's lifetime, and the agent_id is invariant per agent. ~~The recipe is fully deterministic from capture inputs; no agent-side mapping table, no state across restarts.~~ **Amendment 2026-05-28:** the original text assumed both ETW Launch and Terminate EventRecords carry the process creation timestamp in `EVENT_HEADER.TimeStamp`. Empirical validation in Phase 4 Session 16 demonstrated this is false: the Launch EventRecord carries the creation timestamp, but the Terminate EventRecord carries the termination timestamp — a different value (~µs to hours later). Cross-event uid stability is therefore achieved via the `CreatedTimeCache` (SPEC-005 §Operational §2): populated at Launch with the creation_time, consulted at Terminate to supply the same value for `process.uid` construction. This IS an agent-side mapping (PID → creation_time), but it is volatile (in-memory, lost on restart) and PID-keyed (not uid-keyed), consistent with ADR-0009 §Decision part 3's "no on-disk event state" constraint. On cache-miss (Terminate without a prior observed Launch), `process.uid` falls back to the Terminate event's own timestamp — the uid is then not correlatable with a Launch uid, consistent with the `process.created_time = null` emission for the same case.
