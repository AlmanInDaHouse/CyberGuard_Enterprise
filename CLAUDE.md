...
| -------- | -------------- | ------ | ----- | ----------------- |
| `ts-ci` (`check-api` job) | `17b81c7` | SPEC-009 read-slice PART 1/2 (read-API) harness-first RED — the four data/logic read ACs (`read_ac_001/002/003/005`) fail by design (read handlers stubbed `NotImplemented` → 501; certified per-AC as control-absent, not table-missing — the read-target tables are materialised in the api test harness, option (c)). `read_ac_004` (auth/RBAC enforcement) is a green-guard (passes via SPEC-008's `makeRequireSession`). Structure GREEN; SPEC-008's 25 tests no regression. The dashboard RED (PART 2/2) is a separate later SHA. Manuel authorised this RED at the Session 19 gate. | Claude Code / Manuel | closed in the SPEC-009 read-API GREEN impl SHA (row removed there, Convention #13) |
